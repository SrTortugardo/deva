...
#define ATA_REG_DRIVE                                                          \
  6 /* W:    bits 24-27 del LBA + selector master/slave + modo LBA */
...
#define ATA_CMD_IDENTIFY                                                       \
  0xEC /* IDENTIFY DEVICE: devuelve 256 words con info del disco */

/* Bits del registro STATUS (offset 7). */
#define ATA_SR_BSY                                                             \
  0x80 /* BUSY: el disco esta ocupado, no acepta mas comandos */
...
#define ATA_SR_ERR                                                             \
  0x01 /* ERROR: se setea si hubo error en el ultimo comando                   \
        */

#define SECTOR_SIZE 512 /* tamano de un sector ATA estandar */
#define FS_START_SECTOR                                                        \
  2048 /* primer sector donde vive devafs en el disco (1 MiB) */
